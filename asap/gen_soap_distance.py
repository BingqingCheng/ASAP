#!/usr/bin/python3
"""
python3 gen_soap_distance.py --fxyz *.xyz --dict *.xyz --prefix $prefix
...
def main(fxyz, dictxyz, prefix, soap_rcut, soap_g, soap_n, soap_l, soap_periodic, matrix_plot):
    ...
    between reference structures in the dictxyz, and structures in fxyz
    ...
    # save
    np.savetxt(prefix+"-n"+str(soap_n)+"-l"+str(soap_l)+"-c"+str(soap_rcut)+"-g"+str(soap_g)+".dMN",
               dMN, fmt='%4.8f')

    # plot
    if matrix_plot:
        plt.matshow(dNN)
        plt.title('Distance matrix: '+prefix)
        plt.show()
